...
const Renderer = {
    ...
    /*
     * Camera transform target (audit: camera architecture).
     *
     * All "world" layers (everything except `ui`) live inside
     * this container instead of directly on the stage. Camera
     * (js/camera.js) transforms ONLY this container — pan/zoom
     * never touches the `ui` Pixi layer, matching the Master
     * Spec's requirement that camera movement never compromise
     * UI/touch readability.
     *
     * Today Camera is a no-op (x=0, y=0, zoom=1), so this is
     * architecturally present but produces zero visual change —
     * it exists so future camera work (tracking, zoom, launch
     * anticipation) has a single, correct place to apply a
     * transform, rather than needing to touch every layer.
     */
    worldContainer: null,
    ...
    init() {
        ...
        /*
         * World container — see the field comment above.
         * Everything except `ui` attaches here instead of
        ...
        /*
         * World layers attach to worldContainer, which the
         * camera transforms. `ui` attaches directly to the
         * stage, outside the camera transform.
         */
