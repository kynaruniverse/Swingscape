...
// ============================================================
//
// Registered once at load time (not inside init()) since
// Particles is a persistent singleton — re-subscribing inside
// init() would stack duplicate listeners across Game.reset()
// calls, since init() only runs once per session while other
// systems reset per level.
// ============================================================

PresentationEvents.on('launch', payload => {
...
PresentationEvents.on('land', payload => {
    ...
    /*
     * Landing particles fire for every surface, mirroring
     * the pre-event-bus behaviour where Particles.createLanding()
     * was called unconditionally at the top of Pancake.land().
     */
